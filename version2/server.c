...
/*
 *LT 模式：没读完还会提醒你。
 *ET 模式：只提醒一次，所以你必须读到 EAGAIN。<Paste>
 */
...
// 1.socket创建网络通信的文件描述符
// 2.setsockopt端口复用
// 3.bind绑定网络文件描述符与ip和端口
// 4.listen让文件描述符处于监听的状态
// 5.返回监听的文件描述符
 ...
 *struct epoll_event
 *{
 *    uint32_t events;   关心的时间
 *    epoll_data_t data; 关系的数据
 *};
 *
 */

// 1.epoll_create创建一个epoll树，底层是一颗红黑树的。
// 2.epoll_ctl先添加lfd到树上去。
// 3.死循检测
// 3.1监听事件的到来
// 3.2通信时间的到来
...
void* acceptClient(void* arg)
{
  ...
  if(epoll_ctl(info->epfd, EPOLL_CTL_ADD, cfd, &ev) < 0)
  {
    perror("epoll_ctl");
    return NULL;
  }
  printf("acceptClient pthread id : %ld\n", info->tid);
...
// 1.一直读取数据http的数据。
// 2.然后解析get请求的静态资源目
// 注意读取的资源：epoll只会通知你一次，需要自己一次读完的。
...
void* recvHttpRequest(void* arg)
{
  ...
  printf("recvHttpRequest pthread id : %ld\n", info->tid);
  free(info);
  return NULL;
}
